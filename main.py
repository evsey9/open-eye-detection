...
was_opened = True
cur_opened = True
last_cur_opened = True

alarming = False

last_alarmed = time_start
...
while (cap.isOpened()):
    ...
    if ret == True:
        ...
        if eyes_opened != was_opened:
            was_opened = eyes_opened
            last_change = time.time()
            if time.time() - last_static >= eye_delta_timer:
                cur_opened = False
        else:
            if time.time() - last_change >= eye_delta_timer:
                cur_opened = was_opened
